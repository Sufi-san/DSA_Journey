...
public class FindUniqueIn3Rep {
    ...
    // Approach 2 Details:
    // Since Binary Conversion takes only O(logN) time, we get overall O(NlogN) Time Complexity
    ...
    static int findUniqueElem2(int[] arr){
        ...
        int ans = 0;
        for(int elem: arr){
            bitSum += convertToBinary(elem); // converting elements to binary form and adding the bits
        }
        int power = 0;
        while(bitSum > 0){
